...
function transformElement($, file, options) {
  ...
  content = transformScript(content, tmpl, element, !file.optimizer);
...
function transformTemplate(content, element, file) {
  ...
  var fn = 'function tmpl(tag, attrs, refers) { \nvar create = Rosetta.create;\nreturn (<div class={tag.type}>' + content + '</div>);};';
