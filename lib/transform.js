...
function transform(content, file, options) {
  ...
  content = fis.compile.partial(content, file, file.ext);
  ...
  if (element.length) {
    if (element.length > 1) {
      ...
      if (!scripts.length) {
        throw new Error('`script` tag is mandatory for an custom element.');
      }
...
function findElementScript(element, file) {
  ...
  element.children('script').get().reverse().every(function(item) {
    item = cheerio(item);
    var content = item.text();

    // 外链
    if (item.attr('src')) {
      var info = fis.project.lookup(item.attr('src'), file);
...
function transformElement($, file, options) {
  ...
  if (script.attr('src')) {
    f = fis.project.lookup(script.attr('src'), file).file;
...
function transformHTML($, file) {
  ...
  $('*').each(function() {
    ...
    $(this).replaceWith('<textarea id="rs-'+(++index)+'"><![CDATA[\nRosetta.render(' + fn + ', "rs-'+index+'");\n]]></textarea>');
  });
